...
type Value struct {
	// raw_bytes contains the value, a type tag (ValueType) and a
	// checksum. It is custom encoded instead of being split into proto
	// fields to avoid proto overhead and avoid unnecessary encoding and
	// decoding as the value gets read from disk and passed through the
	// network. The format is: <4-byte-checksum><1-byte-tag><value>. A
	// CRC-32-IEEE checksum is computed from the associated key + the
	// tag and value, in that order.
