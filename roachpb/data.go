...
func (v *Value) SetFloat(f float64) {
	v.RawBytes = make([]byte, 5+8)
	encoding.EncodeUint64(v.RawBytes[5:5], math.Float64bits(f))
...
func (v *Value) SetInt(i int64) {
	v.RawBytes = make([]byte, 5+8)
	encoding.EncodeUint64(v.RawBytes[5:5], uint64(i))
...
func (v *Value) SetTime(t time.Time) {
	v.RawBytes = make([]byte, 5, 16)
	v.RawBytes = encoding.EncodeTime(v.RawBytes[0:5], t)
...
func (v Value) GetInt() (int64, error) {
	...
	dataBytes := v.dataBytes()
	if len(dataBytes) != 8 {
		return 0, fmt.Errorf("uint64 value should be exactly 8 bytes: %d", len(dataBytes))
	}
	_, u, err := encoding.DecodeUint64(dataBytes)
	if err != nil {
		return 0, err
