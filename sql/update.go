...
func (p *planner) Update(n *parser.Update) (planNode, error) {
	...
	for _, m := range tableDesc.Mutations {
		if index := m.GetIndex(); index != nil {
			...
			case DescriptorMutation_DELETE_ONLY:
				// Delete index independent of whether needsUpdate.
	...
	for rows.Next() {
		...
		// to eliminate invalid entries in the index. We do not need to
		// compute the new values for these indexes, because we are not allowed
		// to write the new vales into these indexes. One side-effect, is that
		// a key that hasn't changed is also deleted. However, it is safe to
