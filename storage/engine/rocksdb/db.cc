...
DBStatus MVCCComputeStats(DBIterator* iter, DBSlice start, DBSlice end, MVCCStats *stats) {
  ...
  const int64_t now_nanos = stats->last_update_nanos;
  ...
  for (; iter_rep->Valid() && iter_rep->key().compare(end_key) < 0; iter_rep->Next()) {
    ...
    if (!DecodeBytes(&buf, &decoded)) {
      return ToDBString("unable to decoded key");
    }
    ...
    if (!isValue) {
      if (buf.size() != 0) {
        return ToDBString("there should be 12 bytes for encoded timestamp");
      }
      ...
      if (!meta.ParseFromArray(value.data(), value.size())) {
        return ToDBString("unable to decode MVCCMetadata");
      }
      ...
      if (isSys) {
        ...
        if (first) {
          ...
          if (meta.key_bytes() != mvcc_version_timestamp_size) {
            ...
            return ToDBString(rocksdb::Slice(buf, n));
          }
          ...
          if (!DecodeUint64Decreasing(&buf, &wall_time)) {
            return ToDBString("unable to decode mvcc timestamp");
          }
