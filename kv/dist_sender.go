...
// key range w, with the given lookupOptions. The range descriptor
...
func (ds *DistSender) getDescriptors(w window, options lookupOptions) (*roachpb.RangeDescriptor, bool, func(), *roachpb.Error) {
	...
	needAnother := func(desc *roachpb.RangeDescriptor, isReverse bool) bool {
		if isReverse {
			return w.from.Less(desc.StartKey)
		}
		return desc.EndKey.Less(w.to)
	}
...
func (ds *DistSender) sendChunk(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	for {
		...
		for r := retry.Start(ds.rpcRetryOptions); r.Next(); {
			...
			var evictDesc func()
			desc, needAnother, evictDesc, pErr = ds.getDescriptors(w, options)
			descDone()
			...
			if (isReverse && !desc.ContainsKeyRange(desc.StartKey, w.to)) || (!isReverse && !desc.ContainsKeyRange(w.from, desc.EndKey)) {
				evictDesc()
				continue
			}
			...
			curReply, pErr = func() (*roachpb.BatchResponse, *roachpb.Error) {
				...
				untruncate, numActive, trErr := truncate(&ba, desc, w)
